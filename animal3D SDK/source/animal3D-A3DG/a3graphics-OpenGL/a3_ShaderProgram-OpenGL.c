...
a3ret a3shaderProgramAttachShader(a3_ShaderProgram *program, a3_Shader *shader)
{
	...
	//	(VBO)	   ________ ________ ________ ________ ________		(FBO)		 ________	
	// attribs -> |________|FIXEDTCS|FIXEDTES|FIXED GS|________| -> frame		|COMPUTE_| <- compute is separate
	//				^ ^	^								^ ^	^
...
{
	a3ui32 value[1];
	glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_COUNT, dimension % 3, value);
	return *value;
}
...
{
	a3ui32 value[1];
	glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_SIZE, dimension % 3, value);
	return *value;
}
...
{
	a3ui32 value[1];
	glGetIntegerv(GL_MAX_COMPUTE_IMAGE_UNIFORMS, value);
	return *value;
}
